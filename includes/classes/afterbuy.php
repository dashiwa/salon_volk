...
class os_afterbuy_functions {
	var $order_id;

	function os_afterbuy_functions($order_id) {
	...
	function process_order() {
		...
		$o_query = os_db_query("SELECT * FROM ".TABLE_ORDERS." WHERE orders_id='".$oID."'");
		$oData = os_db_fetch_array($o_query);
		...
		$c_query = os_db_query("SELECT customers_gender FROM ".TABLE_CUSTOMERS." WHERE customers_id='".$customer['id']."'");
		$c_data = os_db_fetch_array($c_query);
		...
		$p_query = os_db_query("SELECT * FROM ".TABLE_ORDERS_PRODUCTS." WHERE orders_id='".$oID."'");
		$p_count = os_db_num_rows($p_query);
		...
		$nr = 0;
		$anzahl = 0;
		while ($pDATA = os_db_fetch_array($p_query)) {
			$nr ++;
			...
			$a_query = os_db_query("SELECT * FROM ".TABLE_ORDERS_PRODUCTS_ATTRIBUTES." WHERE orders_id='".$oID."' AND orders_products_id='".$pDATA['orders_products_id']."'");
			$options = '';
			while ($aDATA = os_db_fetch_array($a_query)) {
				if ($options == '') {
		...
		$order_total_query = os_db_query("SELECT
						                      class,
						                      value,
						                      sort_order
		...
		$coupon_flag = false;
		$gv = '';
		while ($order_total_values = os_db_fetch_array($order_total_query)) {
		...
		$b_query = os_db_query("SELECT * FROM ".TABLE_BANKTRANSFER." WHERE orders_id='".$oID."'");

		if (os_db_numrows($b_query)) {
			$b_data = os_db_fetch_array($b_query);
		...
		if (ereg("<success>1</success>", $result)) {
			...
			$cdr = $cdr[0];
			os_db_query("update ".TABLE_ORDERS." set afterbuy_success='1',afterbuy_id='".$cdr."' where orders_id='".$oID."'");
			if ($order_status != '') {
				os_db_query("update ".TABLE_ORDERS." set orders_status='".$order_status."' where orders_id='".$oID."'");
			}
		} else {
	...
	function order_send() {
		...
		$check_query = os_db_query("SELECT afterbuy_success FROM ".TABLE_ORDERS." WHERE orders_id='".$this->order_id."'");
		$data = os_db_fetch_array($check_query);
