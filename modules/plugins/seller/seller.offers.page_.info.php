<?php
    ...
    left join `'.DB_PREFIX.'products` p on (p.products_id=pd.products_id) WHERE s.products_id='.(int)$_GET['products_id'].' and pc.categories_id=29 '.get_zone_id( (int)$_GET['zone_id'], 'and p.zone_id in' ).';');
    ...
    {
        global $osPrice;
        echo '<b>Предложения от '.count($seller_array).' продавца в '.get_zone_block('text').'</b><br><br>';    
        global $osPrice;
        ...
        foreach ($seller_array as $seller_id => $value_seller)
        {
            ...
            <tbody>
            <tr>
            <td style="text-align: center;" width="60px"><div style="padding:5px;"><b>'.$price.'</b></div></td>
            
            <td style="text-align: center;" width="120"><a href="product_info.php?products_id='.$seller_id.'">'.$value_seller['products_name'].'<br><img src="'.$products_image.'" border="0"/></a><br /><span style="font-size: x-small;">'.$value_seller['param_address'].'<br /></span><a href="reviews.php?products_id='.$seller_id.'"><span style="font-size: x-small;">Отзывы ('.$reviews_count.')</span></a></td>
            <td width="150">
            <p align="center"><span style="font-size: x-small;">'.$value_seller['param_phone'].'</span></p>
