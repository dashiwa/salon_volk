...
if (@$_GET['function']) 
   {
	...
	os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$_GET['cPath'].'&action=new_product&pID='.(int) $_GET['pID']));
}
...
if (isset ($_POST['multi_status_on'])) {
	...
	os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$_GET['cPath'].'&'.os_get_all_get_params(array ('cPath', 'action', 'pID', 'cID'))));
}
...
if (@$_GET['action']) 
{
	switch ($_GET['action']) {
		...
		case 'setcflag' :
			...
			os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$_GET['cPath'].'&cID='.$_GET['cID']));
			break;
		...
		case 'setpflag' :
			...
			if ($_GET['pID']) {
				os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$_GET['cPath'].'&page='.$_GET['page'].'&pID='.$_GET['pID']));
			} else {
				os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$_GET['cPath'].'&page='.$_GET['page'].'&cID='.$_GET['cID']));
			}
			break;
			//EOB setpflag
...
//					if ($_GET['flag'] == '1') $catfunc->link_product($_GET['pID'], 0);
			...
			if ($_GET['pID']) {
				...
				os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$_GET['cPath'].'&page='.$_GET['page'].'&cID='.$_GET['pID']));
			}
			break;
		...
		case 'multi_action_confirm' :
			...
			if (isset ($_POST['multi_move_confirm'])) {
				...
				set_categories_url_cache();
	            set_category_cache();
				os_redirect(os_href_link('plugins_page.php', 'page=seller_add_page&cPath='.$dest_category_id.'&'.os_get_all_get_params(array ('cPath', 'action', 'pID', 'cID'))));
			}
