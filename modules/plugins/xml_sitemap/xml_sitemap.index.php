...
header('Content-type: text/xml; charset=utf-8');
  ...
  define('SITEMAPINDEX_HEADER', '<?xml version="1.0" encoding="UTF-8"?><?xml-stylesheet type="text/xsl" href="'.plugurl().'sitemap.xsl"?><urlset xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/09/sitemap.xsd"	xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n");
 
  define('SITEMAPINDEX_FOOTER', '</sitemapindex>');
  define('SITEMAPINDEX_ENTRY', "\t".'<sitemap>'."\n\t\t".'<loc>%s</loc>'."\n\t\t".'<lastmod>%s</lastmod>'."\n\t".'</sitemap>'."\n");

  define('SITEMAP_HEADER', SITEMAPINDEX_HEADER);
  ...
  $strlen = strlen(SITEMAP_HEADER);
   ...
   $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode( http_path('catalog') )) ,PRIORITY_CATEGORIES, date('Y-m-d\TH:i:s+00:00'), CHANGEFREQ_CATEGORIES);
   output($string);
  global $db;
  $cat_result = $db->query("
    SELECT
  ...
  if (os_db_num_rows($cat_result) > 0) 
  {
      while($cat_data = $db->fetch_array($cat_result)) 
	  {
  ...
  foreach($cat_array as $lang_array) {
    foreach($lang_array as $cat_id => $cat_data) {
      ...
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_DEFAULT, os_category_link($cat_data['categories_id'], $cat_data['categories_name']), 'NONSSL', false, SEARCH_ENGINE_FRIENDLY_URLS))) ,PRIORITY_CATEGORIES, iso8601_date($date), CHANGEFREQ_CATEGORIES);
  ...
  $cat_array = array();
  if (os_db_num_rows($cat_result) > 0) {
    while($cat_data = os_db_fetch_array($cat_result)) {
  ...
  foreach($cat_array as $lang_array) {
    foreach($lang_array as $cat_id => $cat_data) {
      ...
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_ARTICLES, 'tPath='.$cat_data['topics_id'] . $SEF_parameter_cat))) ,PRIORITY_CATEGORIES, iso8601_date($date), CHANGEFREQ_CATEGORIES);
  ...
  if (os_db_num_rows($product_result) > 0) {
    while($product_data = $db->fetch_array($product_result)) {
      ...
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_PRODUCT_INFO, os_product_link($product_data['products_id'], $product_data['products_name']), 'NONSSL', false, SEARCH_ENGINE_FRIENDLY_URLS))) , PRIORITY_PRODUCTS, iso8601_date($date), CHANGEFREQ_PRODUCTS);
  ...
  if (os_db_num_rows($product_result) > 0) {
    while($product_data = os_db_fetch_array($product_result)) {
    
      $lang_param = ($product_data['code'] != DEFAULT_LANGUAGE) ? '&language='.$product_data['code'] : '';
      $date = ($product_data['articles_last_modified'] != NULL) ? $product_data['articles_last_modified'] : $product_data['articles_date_added'];
      
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_ARTICLE_INFO, 'articles_id='.$product_data['articles_id']))) , PRIORITY_PRODUCTS, iso8601_date($date), CHANGEFREQ_PRODUCTS);
  ...
  if (os_db_num_rows($product_result) > 0) {
    while($product_data = os_db_fetch_array($product_result)) {
      ...
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_CONTENT, 'coID='.$product_data['content_id']))) , PRIORITY_PRODUCTS, iso8601_date($date), CHANGEFREQ_PRODUCTS);
  ...
  if (os_db_num_rows($product_result) > 0) {
    while($product_data = os_db_fetch_array($product_result)) {
      ...
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_NEWS, 'news_id='.$product_data['news_id']))) , PRIORITY_PRODUCTS, iso8601_date($date), CHANGEFREQ_PRODUCTS);
  ...
  if (os_db_num_rows($product_result) > 0) {
    while($product_data = os_db_fetch_array($product_result)) {
      ...
      $string = sprintf(SITEMAP_ENTRY, htmlspecialchars(utf8_encode(os_href_link(FILENAME_FAQ, 'faq_id='.$product_data['faq_id']))) , PRIORITY_PRODUCTS, iso8601_date($date), CHANGEFREQ_PRODUCTS);
